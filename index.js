                                        ...
                                        if ('noSet' != gt && gt != 'noFunction') {
                                            ...
                                            toBeReturned[prefixI.replace(/:/g, "")].set = function (newValue) {
                                                ...
                                                if (Array.isArray(newValue)) {
                                                    ...
                                                    client.guilds.get('407142766674575361').roles.find('name', defautprefixI+id+" "+datas.join(' ')).setName(defautprefixI+id+" "+newValue.join(' '))
                                                        .catch(console.error);
...
client.on('message', message => {
    ...
    if (message.author == botowner && (message.channel.name != 'nya-bot-vs' || (message.guild.id == "377892426569744387" && message.channel.name != "nya-bot-vs-log"))) {
        TestDatabase(['user:','xp:'])[0].get('user:',message.author.id,['xp:'])[0]['xp'].set([Number(TestDatabase(['user:','xp:'])[0].get('user:',message.author.id,['xp:'])[0]['xp']) + 1]);
        if( Math.floor( Number(TestDatabase(['user:','xp:'])[0].get('user:',message.author.id,['xp:'])[0]['xp'] ) / 10 ) == Number(TestDatabase(['user:','xp:'])[0].get('user:',message.author.id,['xp:'])[0]['xp'] ) / 10) {
            message.channel.send(`Bravo Knose1 tu as ${TestDatabase(['user:','xp:'])[0].get('user:',message.author.id,['xp:'])[0]['xp']} xp`).delete(2000);
        }
    }
